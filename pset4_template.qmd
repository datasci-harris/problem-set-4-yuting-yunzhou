...
=======
>>>>>>> 2b3831d9f0472a3ec00e1224ef0deb3c1ce3070c
```{python}
import pandas as pd
...
print(f"Unique ZIP codes in Texas subset: {zips_texas_centroids['ZCTA5'].nunique()}")
...
zips_texas_centroids = gpd.GeoDataFrame(zips_texas_centroids, geometry='centroid')
...
def calculate_nearest_distance(row, hospitals_gdf):
    nearest_geom = nearest_points(row.geometry, hospitals_gdf.unary_union)[1]
    distance_meters = row.geometry.distance(nearest_geom)
    distance_miles = distance_meters * 0.000621371  
    return distance_miles

zips_texas_centroids['nearest_hospital_dist_miles'] = zips_texas_centroids.apply(
    calculate_nearest_distance, hospitals_gdf=zips_withhospital_centroids, axis=1
)


from shapely.ops import nearest_points
def calculate_nearest_distance(row, hospitals_gdf):
    # Find the nearest point in hospitals_gdf for each ZIP code point
    nearest_geom = nearest_points(row.geometry, hospitals_gdf.unary_union)[1]
